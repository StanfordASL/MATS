...
class MultiNode(Node):
    def __init__(self, node_type, node_id, nodes_list, hyperparams, is_robot=False):
        super(MultiNode, self).__init__(node_type, node_id, data=None, is_robot=is_robot)

        mhl = hyperparams['minimum_history_length']
        ph = hyperparams['prediction_horizon']

        self.nodes_list = nodes_list
        self.timestep_node_dict = dict()
        for node in self.nodes_list:
            node.is_robot = is_robot
            self.timestep_node_dict.update({t: node for t in range(node.first_timestep + mhl,
                                                                   node.last_timestep - ph + 1)})
    ...
    def get_node_at_timesteps(self, scene_ts) -> Node:
        single_ts = (scene_ts[0] + scene_ts[1]) // 2
        if single_ts not in self.timestep_node_dict:
            return Node(node_type=self.type,
                        node_id='EMPTY',
                        data=self.nodes_list[0].data * np.nan,
                        is_robot=self.is_robot)

        return self.timestep_node_dict[single_ts]
    ...
    def scene_ts_to_node_ts(self, scene_ts) -> (Node, np.ndarray, int, int):
        ...
        node = self.timestep_node_dict[single_ts]
    ...
    def history_points_at(self, ts) -> int:
        ...
        :return: Number of history timesteps.
        """
        return ts - self.timestep_node_dict[ts].first_timestep
