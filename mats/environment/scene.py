...
class Scene(object):
    def __init__(self, timesteps, map=None, dt=1, name="", frequency_multiplier=1, aug_func=None,  non_aug_scene=None,
                 sample_tokens=None, x_min=None, y_min=None):
        self.map = map
    ...
    def add_robot_from_nodes(self, robot_type, hyperparams, min_timesteps=3):
        nodes_list = [node for node in self.nodes if node.type == robot_type]
        non_overlapping_nodes = MultiNode.find_non_overlapping_nodes(nodes_list, min_timesteps=min_timesteps)
        self.robot = MultiNode(robot_type, '0', non_overlapping_nodes, hyperparams, is_robot=True)
        ...
        if hasattr(self, 'augmented'):
            for augmented_scene in self.augmented:
                augmented_scene.add_robot_from_nodes(robot_type, hyperparams, min_timesteps=min_timesteps)
    ...
    def sample_timesteps(self, batch_size, min_future_timesteps=0, min_history_length=0) -> np.ndarray:
        """
        ...
        """
        timesteps = np.arange(min_history_length, self.timesteps-min_future_timesteps)
        if batch_size > timesteps.shape[0]:
            batch_size = timesteps.shape[0]
        return np.random.choice(timesteps,
